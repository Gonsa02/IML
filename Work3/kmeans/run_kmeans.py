...
def kmeans_sort_csv():
    ...
    #     # Load Existing Results
    #     try:
    #         kmeans_csv_file = f'results/kmeans_{dataset_name}_results.csv'
    #         kmeans_df = pd.read_csv(kmeans_csv_file)
    #     except FileNotFoundError:
    #         kmeans_df = pd.DataFrame()
    ...
    # for k, distance, seed in itertools.product(k_values, distance_metrics, seeds):
    #     params = (dataset_name, k, distance, seed)
    #     result = process_combination(params, datasets)
    ...
    #                                 distance=distance, seed=seed, verbose=False)
    #                                 predicted_labels = kmeans.fit_predict(X_scaled)

    #                                 # Compute accuracy
    #                                 accuracy = KMeans.compute_accuracy(
    #                                     predicted_labels, y_true)

    #                                 # Optionally, compute additional metrics like inertia (sum of squared distances)
    #                                 # Here, we'll compute inertia using the chosen distance metric
    #                                 if distance == 'euclidean':
    #                                 # Inertia for Euclidean is the sum of squared distances
    #                                 inertia = np.sum(
    #                         (X_scaled - kmeans.centroids[predicted_labels]) ** 2)
    #                         elif distance == 'manhattan':
    #                         inertia = np.sum(
    #                         np.abs(X_scaled - kmeans.centroids[predicted_labels]))
    #                         elif distance == 'cosine':
    #                         # For cosine, inertia can be defined differently; here we'll use sum of cosine distances
    #                         inertia = np.sum(Distances.cosine_distance(
    #                         X_scaled, kmeans.centroids[predicted_labels]))
    #                         else:
    #                     inertia= None  # Should not happen
