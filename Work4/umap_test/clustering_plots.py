...
def plot_UMAP(df, labels, title, show_legend=True):
    n_neighbors = 15
    um = UMAP(n_neighbors=n_neighbors, min_dist=0.1, n_components=2)
...
def run_experiments():
    ...
    for dataset_name, dataset_content in preprocessed_datasets.items():
        ...
        plot_PCA(data, y_pred_optics, dataset_name)
        plot_UMAP(data, y_pred_optics, f"{dataset_name} OPTICS clustering")
...
def umap_reduce_and_cluster():
    ...
    for dataset_name, dataset_content in preprocessed_datasets.items():
        ...
        # Perform UMAP reduction and clustering
        for seed in seeds:
            start = time.time()
            ...
            # OPTICS Clustering
            if dataset_name == 'satimage':
                optics_y_pred = compute_optics(data, "euclidean", "brute", 2)
            elif dataset_name == 'splice':
                optics_y_pred = compute_optics(data, "l1", "brute", 2)
            else:
                optics_y_pred = compute_optics(data, "euclidean", "brute", 4)
...
def pca_reduce_and_cluster():
    ...
    for dataset_name, dataset_content in preprocessed_datasets.items():
        ...
        # Perform PCA reduction and clustering
        start = time.time()
        reducer = PCA.imlPCA()
        reduced_data = reducer.fit_transform(data)
        end = time.time()
